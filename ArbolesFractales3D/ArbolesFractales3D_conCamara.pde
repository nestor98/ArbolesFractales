...
float yOffset = 11*height/12;


void dibujarRamas(float len, float weight, int n) {
  if (n>0) {
     strokeWeight(weight);
     //Rama raiz:
     line (0,0,0,-len);
     ...
     rotateY(2.0*PI/3);
     rotateZ(deg+deg1);
     dibujarRamas(conversion*len, weight*weightConv, n-1);
     popMatrix();
     //Rama 2
     pushMatrix();
     rotateY(4.0*PI/3);
...
void draw() {
   background(0);
   pushMatrix();
   translate(xOffset, yOffset, 0); // Principio
   
   rotateX(rotX); // Control con el raton
   rotateY(rotY); 
   scale(zoom);
   dibujarRamas(longitud, weight, 9); // Se dibuja el arbol (con 3^9 ramas)
   popMatrix();
   if (show){
